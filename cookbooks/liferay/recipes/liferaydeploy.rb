...
powershell_script 'Unzip Liferay package' do
  ...
  code <<-EOH
    powershell.exe -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('liferay-base-install-6.1.30.zip', 'C:\liferay\')}"
  EOH
